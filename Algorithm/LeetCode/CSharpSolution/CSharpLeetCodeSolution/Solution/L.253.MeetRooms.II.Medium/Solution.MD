...
![](img/253_Meeting_Rooms_II_Diag_1.png)
...
![](img/253_Meeting_Rooms_II_Diag_2.png)
...
![](img/253_Meeting_Rooms_II_Diag_3.png)
...
![](img/253_Meeting_Rooms_II_Diag_4.png)

![](img/253_Meeting_Rooms_II_Diag_5.png)
...
- Time Complexity: O(N\log N)*O*(*N*log*N*) because all we are doing is sorting the two arrays for `start timings` and `end timings` individually and each of them would contain N*N* elements considering there are N*N* intervals.
- Space Complexity: O(N)*O*(*N*) because we create two separate arrays of size N*N*, one for recording the start times and one for the end times.